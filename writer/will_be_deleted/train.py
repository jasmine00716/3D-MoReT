...
def calculate_validate_loss():
    ...
    with torch.no_grad():
        for index, (inputs, labels, label_weight, mask, file) in enumerate(validation_loader):
            ...
            predicts = network(inputs)
            loss = 0
            if len(val_loss_list) == 1: # loss function 1개만 사용할 경우
...
if __name__ == '__main__':
    ...
    # network = load_nest_config(**config.network_architecture)
    from models.MobileViT.mobilevit import Mobilevit_nearest_neighbor_upsampling
    network = Mobilevit_nearest_neighbor_upsampling(image_size=(240, 240),
                                          dims=[64, 80, 96],
                                          channels=[64, 128, 256, 512, 1024],
                                          # [16, 24, 40, 112, 1280], [16, 64, 128, 512, 1024](design1-채널수변경)
                                          kernel_size=3,
                                          patch_size=(1, 2, 2),
                                          num_classes=7)
    # network = nn.parallel.DistributedDataParallel(network).to(torch.device("cuda:7"))
    ...
    #
    # toy_input = torch.randn(1, 40, 20, 224, 224).to('cpu')  # create a random input
    # network = network.to('cpu')
    # output = network(toy_input)
    # print('params', output)
    # flops, params, ret_layer_info = profile(network.to('cpu'), inputs=(toy_input.to('cpu'),), ret_layer_info=True)
    # print('params', params)
    # print('FLOPs:', flops)
    #
    # for key, item in ret_layer_info.items():
    #     print(key, item)
    breakpoint()
