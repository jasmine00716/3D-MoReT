...
# from models.Unet_3D.buildingblocks import DoubleConv, ResNetBlock, ResNetBlockSE, create_decoders, create_encoders
from utils import get_class, number_of_features_per_level
# from models.Unet_3D.utils import get_class, number_of_features_per_level
...
class AbstractUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        self.encoders = create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding, layer_order,
                                        num_groups, pool_kernel_size, is3d).to(dev_0)
        ...
        if is3d:
            self.final_conv = nn.Conv3d(f_maps[0], out_channels, 1).to(dev_0)
        else:
            self.final_conv = nn.Conv2d(f_maps[0], out_channels, 1).to(dev_0)

        if is_segmentation:
            # semantic segmentation problem
            if final_sigmoid:
                self.final_activation = nn.Sigmoid().to(dev_0)
            else:
                self.final_activation = nn.Softmax(dim=1).to(dev_0)
        else:
            # regression problem
...
def get_model(model_config):
    model_class = get_class(model_config['name'], modules=['unet3d'])
...
if __name__ == '__main__':
    ...
    network = UNet3D(in_channels=40, out_channels=5).to(device)

    out = network(img)
    print(out.shape)

    totalparams = count_parameters(network)
    ...
    #                                         inputs=(img.to(torch.device("cuda:4")),), ret_layer_info=True)
    # print('params', params)
    # print('FLOPs:', flops)
