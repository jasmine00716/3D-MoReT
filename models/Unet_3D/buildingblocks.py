...
# from models.Unet_3D.se import ChannelSELayer3D, ChannelSpatialSELayer3D, SpatialSELayer3D
...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding, is3d):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            modules.append(('DropOut', nn.Dropout3d(0.5)))  # dropout 추가
        elif char == 'l':
...
class ResNetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = nn.Dropout3d(0.5)(out)  # dropout 추가
        return out
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 ...
                 dev_0=torch.device("cuda:4"), dev_1=torch.device("cuda:5"), dev_2=torch.device("cuda:5"),
        ...
        if apply_pooling:
            if pool_type == 'max':
                if is3d:
                    self.pooling = nn.MaxPool3d(kernel_size=pool_kernel_size).to(dev_0)
                else:
                    self.pooling = nn.MaxPool2d(kernel_size=pool_kernel_size).to(dev_0)
            else:
                if is3d:
                    self.pooling = nn.AvgPool3d(kernel_size=pool_kernel_size).to(dev_0)
                else:
                    self.pooling = nn.AvgPool2d(kernel_size=pool_kernel_size).to(dev_0)
        else:
            self.pooling = None
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
        ...
        if upsample:
            if basic_module == DoubleConv:
                ...
                self.upsampling = InterpolateUpsampling(mode=mode).to(dev_0)
                # concat joining
                self.joining = partial(self._joining, concat=True)
            else:
                # if basic_module=ResNetBlock use transposed convolution upsampling and summation joining
                self.upsampling = TransposeConvUpsampling(in_channels=in_channels, out_channels=out_channels,
                                                          kernel_size=conv_kernel_size, scale_factor=scale_factor).to(dev_0)
                # sum joining
            ...
            # no upsampling
            self.upsampling = NoUpsampling().to(dev_0)
            # concat joining
            self.joining = partial(self._joining, concat=True)
    ...
    def forward(self, encoder_features, x):
        ...
        # print(x.shape, "(upsampling)")
        x = self.joining(encoder_features, x)
        # print(x.shape, "(joining)")
        x = self.basic_module(x)
        # print(x.shape)
        return x
