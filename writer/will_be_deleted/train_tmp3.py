...
Mobilevit_with_3mvit_block, deconvolution
...
from utils_ import load_nest_config, EarlyStopping
...
def calculate_validate_loss():
    ...
    # early_stopping 객체 선언(3번의 epoch 연속으로 loss 미개선 시에 조기 종료)
...
if __name__ == '__main__':
    ...
    network = models.MobileViT_3D.mobilevit.Mobilevit_with_3mvit_block(**params)
    # network = nn.parallel.DistributedDataParallel(network).to(torch.device("cuda:5"))
    ...
    loss_list = []
    loss_weights = [1]
    config.loss_fn['parameters']['device'] = torch.device("cuda:5")
    ...
    config.val_loss_fn['parameters']['device'] = torch.device("cuda:5")
