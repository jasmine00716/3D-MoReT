...
from models.MobileViT_v3_3D.mobilevit_v3_block_advanced_ver import MobileViTBlock as MobileViTBlockAdv
from models.MobileViT_v3_3D.vit_block import SimpleViT
# from models.MobileViT_v3_3D.vit_block_advanced_ver import SimpleViT
# from mobilevit_v3_block import MobileViTBlock
# from vit_block_advanced_ver import SimpleViT
...
class Mobilevit(nn.Module):
    def __init__(self, image_size, dims, channels, num_classes, expansion=4, kernel_size=3, patch_size=(2, 2, 2), device=torch.device("cuda:5")):
        super().__init__()
        ...
        # self.vit1 = SimpleViT(dims[0], L[0], 128, [(2, 28, 2), (2, 2, 20), (20, 2, 2)], heads=4, dim_head=3).to(device)
        self.mvit.append(MobileViTBlock(dims[0], L[0], channels[6], kernel_size, patch_size, int(dims[0] * 2)).to(device))
        # self.mvit.append(MobileViTBlock(dims[0], L[0], channels[6], kernel_size, patch_size, channels[6]*4, dim_head=int(dims[0] * 2)).to(device)) # x
        self.mvit.append(MobileViTBlockAdv(dims[1]//3, L[1], channels[8], kernel_size, [(2, 14, 2), (2, 2, 14), (20, 2, 2)], int(dims[1] * 4)).to(device))
        # self.vit2 = SimpleViT(dims[1], L[1], channels[8], [(2, 14, 2), (2, 2, 14), (20, 2, 2)], heads=4, dim_head=3).to(device)
        ...
        # 384
        self.deconv1 = nn.ConvTranspose3d(128, channels[10], kernel_size=(3, 3, 3), stride=(1, 3, 3), padding=(1, 2, 2)).to(device)  # 640 + 320
