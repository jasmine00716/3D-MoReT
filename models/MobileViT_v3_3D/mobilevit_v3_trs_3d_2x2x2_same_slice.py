...
from models.MobileViT_v3_3D.mobilevit_v3_block import MobileViTBlock
from models.MobileViT_v3_3D.vit_block import SimpleViT
# from mobilevit_v3_block import MobileViTBlock
# from vit_block import SimpleViT
    ...
    def __init__(self, inp, oup, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), expansion=4):
        ...
        if expansion == 1:
            self.conv = nn.Sequential(
                # dw
                nn.Conv3d(hidden_dim, hidden_dim, (1, 3, 3), stride=(1, stride, stride), padding=padding, groups=hidden_dim, bias=False),
...
class Mobilevit(nn.Module):
    def __init__(self, image_size, dims, channels, num_classes, expansion=4, kernel_size=3, patch_size=(2, 2, 2), device=torch.device("cuda:6")):
        ...
        # self.mvit.append(MobileViTBlock(dims[0], L[0], channels[6], kernel_size, patch_size, channels[6]*4, dim_head=int(dims[0] * 2)).to(device))
        self.mvit.append(MobileViTBlock(dims[1], L[1], channels[8], kernel_size, patch_size, int(dims[1] * 4)).to(device))


        # self.mvit.append(MobileViTBlock(dims[2], L[2], 128, kernel_size, (1,6,6), int(dims[2] * 4)).to(device))

        # self.vit = SimpleViT(dims[2], L[2], channels[10], patch_size, heads=4, dim_head=int(dims[2]*4)).to(device)
        self.convMtoN = conv_1x1_gn(channels[10], 128).to(device)  # channels[10]//2
        self.vit = SimpleViT(dims[2], L[2], 128, (2, 2, 2), heads=4, dim_head=32).to(device)
        self.convNtoM = conv_1x1_gn(128, channels[10]).to(device)

        # self.conv2 = conv_nxn_gn(channels[10], channels[12], stride=1).to(device)
        ''''''
        ...
        # self.deconv1 = nn.ConvTranspose3d(channels[11], channels[10], kernel_size=(1, 3, 3), stride=(1, 3, 3), padding=(0, 2, 2)).to(device)  # channels[10], 384
        # self.deconv2 = nn.ConvTranspose3d(2 * (channels[8] + channels[10]), channels[8], kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(device)  # 2*(256 + 320), 256
        # self.deconv3 = nn.ConvTranspose3d(2 * (channels[6] + channels[8]), channels[5], kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(device)  # 2 * (128 + 256), 64
        # self.deconv4 = nn.ConvTranspose3d(2 * (2 * channels[4]), channels[2], kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(device)  # 2 * (64 + 64), 52
        # self.deconv5 = nn.ConvTranspose3d(2 * (2 * channels[2]), 48, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(device)  # 2 * (52 + 52), 4
        ...
        # self.resblock1 = ResidualBlock(480).to(device)  # 384 + 96
        # self.resblock2 = ResidualBlock(240).to(device)  # 192 + 48
        # self.resblock3 = ResidualBlock(120).to(device)  # 96 + 24
        # self.resblock4 = ResidualBlock(60).to(device)  # 48 + 12
        #
        # self.deconv1 = nn.ConvTranspose3d(channels[10], 384, kernel_size=(1, 3, 3), stride=(1, 3, 3), padding=(0, 2, 2)).to(device)  # channels[10], 384
        # self.deconv2 = nn.ConvTranspose3d(2 * 480, 192, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(device)  # 2*(256 + 320), 256
        # self.deconv3 = nn.ConvTranspose3d(2 * 240, 96, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(device)  # 2 * (128 + 256), 64
        # self.deconv4 = nn.ConvTranspose3d(2 * 120, 48, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(device)  # 2 * (64 + 64), 52
        # self.deconv5 = nn.ConvTranspose3d(2 * 60, 24, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(device)  # 2 * (52 + 52), 4
        # self.last_conv = nn.Conv3d(24, 5, kernel_size=(1, 1, 1)).to(device)
    ...
    def forward(self, x):  # (1, 40, 20, 224, 224)
        ...
        # mvit_2 = self.mvit[2](conv256to128)

        # conv2 = self.conv2(conv128to256)  # (1, 640, 20, 6, 6)
        ...
        x = torch.cat((deconv3, mv2_2), dim=1)  # (1, 128, 20, 56, 56)
        # print("x", x.shape)
...
if __name__ == '__main__':
    ...
    img = torch.rand(1, 40, 20, 224, 224)  # (batch_size, time(==channel), slice, height, width)
    ...
    network_architecture = {
        "parameters": {
            ...
            "dims": [64, 80, 96],
            # "channels": [52, 52, 52, 64, 64, 64, 96, 96, 192, 192, 384, 384],
            # [52, 52, 52, 64, 64, 64, 128, 128, 256, 256, 320, 320, 640], #[16, 64, 128, 256, 512],  #[16, 16, 24, 24, 48, 48, 64, 64, 80, 80], # , # [16, 64, 128, 512, 1024](design1-채널수변경), [64, 128, 256, 512, 1024]
